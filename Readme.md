...
**Variant 2: do something after a batch of events is processed, by implementing `AppliedEventsStorageInterface::saveHighestAppliedSequenceNumber`.**

This can be implemented in the following steps:

- Your projector should implement `Neos\EventSourcing\EventListener\AppliedEventsStorage\AppliedEventsStorageInterface`
- If you like, the projector internally can delegate to `DoctrineAppliedEventsStorage` for functionality.
- After the batch has been processed, `AppliedEventsStorageInterface::saveHighestAppliedSequenceNumber` is triggered; where you
  can add your custom logic.

A full example can be found in the code docs of [`AppliedEventsStorageInterface`](./Classes/EventListener/AppliedEventsStorage/AppliedEventsStorageInterface.php).
